		...
		function Elements:AddChatClient()
			...
			local function SendMessage()
				...
				task.delay(0.5, function()
					CreateMessage("AI", "Chamber v2.0 :: Packet received. Protocol stabilized.")
				end)
			end
			...
			local chatApi = {}
			chatApi.SendMessage = function(type:"AI"|"USER",msg)
				if type == "AI" then
			...
			chatApi.EditMessage = function(msgFrame:Frame,newText:string)
				local maxWidth = math.min(ChatScroll.AbsoluteSize.X * 0.78, 360)
				...
				if not typeof(newText) == "string" then return end
			...
			chatApi.DeleteMessage = function(msgFrame:Frame)
				local isUser = msgFrame:GetAttribute("Sender") == "User"
				local Bubble = msgFrame:FindFirstChild("ChatBubble")
				if not Bubble then return end

				local targetPos:UDim2 = if isUser then UDim2.fromScale(1.5,0) else UDim2.fromScale(-1.5,0)
